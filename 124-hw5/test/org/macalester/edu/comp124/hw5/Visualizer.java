...
public class Visualizer extends CanvasWindow {
    ...
    private void visualizeSteps(){
        ...
        List<Point> rotated = recognizer.rotateTo(resampled, -recognizer.findIndicativeAngle(resampled));
        drawPath(rotated, Color.RED);

        List<Point> scaled = recognizer.scaleToSquare(rotated, 100);
        drawPath(scaled, Color.GREEN);

        List<Point> translated = recognizer.translateToPoint(scaled, new Point(0.0,0.0));
